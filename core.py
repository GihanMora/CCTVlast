...
def remove_tracker(track_index, tracker_locations):
    ...
    if track_index < len(static_variables.trackers_list):
        ...
        #static_variables.trackers_list.remove(static_variables.trackers_list[track_index])
        #static_variables.tracker_queues_list.remove(static_variables.tracker_queues_list[track_index])
    ...
    if len(tracker_locations) > len(detected_coordinates):
        ...
        for k in detected_coordinates:
            ...
            if not_matching:
                ...
                print "detection is mismatching... comparing with lost trackers", static_variables.lost_trackers
                for ltr in static_variables.lost_trackers:
                    #if life of lost tracker is grater than 10 frames it is deleted
                    if ((Frame_count - ltr[2]) > 10):
                        remove_tracker(ltr[0], tracker_locations)
                        number_of_removed_trackers+=1
                ...
                if (not (is_lost)):
                    print "This detection is not a lost one... new tracker needed to be initiated"
                    ...
                    if (len(detected_coordinates) > len(tracker_locations)+number_of_removed_trackers):
                        print "diga", detected_coordinates, tracker_locations
...
class VideoCamera(object):
    ...
    def get_frame(self):
        ...
        print "lost tracker", static_variables.lost_trackers
        ...
        for info in static_variables.entrance_information:
            ...
            static_variables.info_set.append(
                "Tracker " + str(info[1]) + " entered to reigon " + str(info[0]) + " at " + str(info[2]))

        if (self.Frame_count % 20 == 0):

            testdb.insertdata(self.Frame_count,people_count, tracker_locations,
                       moving_directions,str(current_time),static_variables.session_number)
            reg_1=count_people_in_reigon([0,0,200,213],coordinates)
            reg_2 = count_people_in_reigon([200, 0, 400, 213], coordinates)
            reg_3 = count_people_in_reigon([400, 0, 600, 213], coordinates)
            print reg_1,reg_2,reg_3
            testdb.insert_region_count(self.Frame_count, reg_1,reg_2,reg_3,
                                        static_variables.session_number)

        try:
            if(static_variables.restricted.__len__()!=0 and static_variables.info_set.__len__()!=0):
                testdb.insert_entrance_data(self.Frame_count,static_variables.restricted,static_variables.info_set,static_variables.session_number)
        except:
            print "sasssasas"
        ...
        cv2.putText(text_box, "Selected Areas: " + str(static_variables.restricted), (50, 300),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (25, 0, 51), 1)
        #cv2.line(frame, (200, 0), (200, 20), (25, 0, 51), 2)
        #cv2.line(frame, (400, 0), (400, 20), (25, 0, 51), 2)
        #cv2.putText(frame, "Region 1", (80, 10),
        #            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (25, 0, 51), 1)
        #cv2.putText(frame, "Region 2", (280, 10),
        #            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (25, 0, 51), 1)
        #cv2.putText(frame, "Region 3", (480, 10),
        #            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (25, 0, 51), 1)
        ...
        fps.update()
        frame1 = cv2.resize(frame, (600,337))
        text_box1 = cv2.resize(text_box,(600, 337))

        frame = np.hstack([frame1, text_box1])
        if static_variables.snap==True:
            snap_name=str(datetime.now(Sri_lanka).microsecond)+'.bmp'
            print snap_name
            cv2.imwrite(r'snapshots/%s'%(snap_name),frame)

            print "saved"

            static_variables.snap=False
